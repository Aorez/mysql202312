...
-- mysql排序
-- using filesort
-- 通过表的索引或者全表扫描，读取满足条件的数据行，然后在排序缓冲区sort buffer中完成排序操作，而不是通过索引直接返回排序结果。
-- using index
-- 通过顺序扫描有序索引直接返回排序结果
-- 
-- age、phone都没有索引，因此是filesort排序。
...
EXPLAIN SELECT id,age,phone FROM tb_user ORDER BY age \g;
EXPLAIN SELECT id,age,phone FROM tb_user ORDER BY age,phone \g;
-- 降序排序
-- 预计Extra中有Backward index scan，但是没有，可能MySQL8才有
-- 意思是索引是从小到大排序的，降序排序需要反向扫描
EXPLAIN SELECT id,age,phone FROM tb_user
ORDER BY age DESC,phone DESC \g;
-- 排序也要求最左前缀匹配原则
-- Extra中有using index，也有using filesort
EXPLAIN SELECT id,age,phone FROM tb_user
ORDER BY phone,age \g;
-- 由于索引默认升序，一个升序一个降序也不可以
EXPLAIN SELECT id,age,phone FROM tb_user
ORDER BY age ASC,phone DESC \g;
-- show index中的collation列中A表示升序，D表示降序
-- phone仍然是asc，可能MySQL8才有
...
GROUP BY profession \g;
-- 创建索引，再进行分组
-- using index
CREATE INDEX idx_user_pro_age_sta on tb_user(profession,age,status);
SHOW INDEX FROM tb_user;
EXPLAIN SELECT profession,COUNT(*) FROM tb_user GROUP BY profession \g;
-- using index
EXPLAIN SELECT profession,COUNT(*) FROM tb_user GROUP BY profession,age \g;
-- using temporary
-- 所以有最左前缀匹配原则的要求
EXPLAIN SELECT age,COUNT(*) FROM tb_user GROUP BY age \g;



-- limit优化
-- tb_sku表数据量很大
-- 进行limit 200000 10时，需要排序200010的数据再取最后十条，性能较差
SELECT * FROM tb_sku ORDER BY id LIMIT 200000,10;
-- 先把十条数据的id查询出来，这个查询覆盖索引，然后再查询数据。
SELECT *
FROM tb_sku s,
(SELECT id FROM tb_sku ORDER BY id LIMIT 200000,10) k
WHERE s.id=k.id;
